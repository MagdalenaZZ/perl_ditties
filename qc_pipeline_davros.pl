...
my $cwd = cwd();
my $masterspace="/mnt/scratch/DCT/BIOINFCO/mzarowiecki/REFERENCES";
...
print OUT "perl /mnt/scratch/DCT/BIOINFCO/mzarowiecki/bin/perl/fastqc_parser.pl $prefix1.F $fol/$prefix1.F.sub1000000_fastqc/fastqc_data.txt\n";
print OUT "\n";
...
print OUT "rm -f  $fol/$prefix1.F.sub1000000_fastqc/fastqc.fo\n";
print OUT "rm -f  $fol/$prefix1.F.sub1000000_fastqc/fastqc_report.html\n";
print OUT "rm -fr  $fol/$prefix1.F.sub1000000_fastqc/Icons/\n";
print OUT "rm -fr  $fol/$prefix1.F.sub1000000_fastqc/Images/\n";
print OUT "rm -f  $fol/$prefix1.F.sub1000000_fastqc.zip\n";
print OUT "gzip -f -r $fol\n";
print OUT "mv $fol/*/* $fol\n";
print OUT "rm -fr $fol/$prefix1.F.sub1000000_fastqc/\n";
...
open (OUT, ">$prefix1.R.qc.sh3") || die "I can't open $prefix1.R.qc.sh3\n";
...
print OUT "source activate py3.6\nmodule add fastqc/0.11.4\nmodule add bowtie2/2.2.6\nmodule add samtools/1.3.1\nmodule add java/sun8/1.8.0u66\nmodule add picard-tools/2.8.1\n";
# Do initial read length histogram
print OUT "# Jobs for getting read length, running FastQC, parsing the output and then tidying up\n";
...
print OUT "perl /mnt/scratch/DCT/BIOINFCO/mzarowiecki/bin/perl/fastqc_parser.pl $prefix2.R $fol/$prefix2.R.sub1000000_fastqc/fastqc_data.txt\n";
print OUT "\n";

#print OUT "Hi\n";

# clean up after fastqc
print OUT "rm -f  $fol/$prefix2.R.sub1000000_fastqc/fastqc.fo\n";
print OUT "rm -f  $fol/$prefix2.R.sub1000000_fastqc/fastqc_report.html\n";
print OUT "rm -fr  $fol/$prefix2.R.sub1000000_fastqc/Icons/\n";
print OUT "rm -fr  $fol/$prefix2.R.sub1000000_fastqc/Images/\n";
print OUT "rm -f  $fol/$prefix2.R.sub1000000_fastqc.zip\n";
print OUT "gzip -f -r $fol\n";
print OUT "mv $fol/*/* $fol\n";
print OUT "rm -fr $fol/$prefix2.R.sub1000000_fastqc/\n";
...
if ($in2=~/SINGLE/) {
print OUT "bowtie2  -p 1 --un-gz $prefix1.un.gz -x $masterspace/MASTER -U $prefix1.F.sub1000000.fq.gz -S $inh1.sam \n";
...
print OUT "infer_experiment.py -i  $prefix1.bam -r ~/scratch/REFERENCES/MASTER.gene.bed > $prefix1.ie\n";
...
print OUT "featureCounts -s 0 --largestOverlap --minOverlap 10 --primary -C  -a $masterspace/MASTER.gtf.fc.ref -o $prefix1.fc $prefix1.bam \n"; 
...
print OUT "module add java/sun8/1.8.0u66\nmodule add picard-tools/2.8.1\nmodule add samtools/1.3.1\n";
print OUT "java -Xmx7g  -XX:ParallelGCThreads=1 -jar \$PICARDHOME/picard.jar SortSam SORT_ORDER=coordinate INPUT=$prefix1.bam OUTPUT=$prefix1.sort.bam\n";
print OUT "java -Xmx7g  -XX:ParallelGCThreads=1 -jar \$PICARDHOME/picard.jar CollectMultipleMetrics INPUT=$prefix1.sort.bam REFERENCE_SEQUENCE=$masterspace/MASTER.fa OUTPUT=$cwd/$prefix1.sort.PMM PROGRAM=CollectGcBiasMetrics\n";

print OUT "samtools flagstat $prefix1.sort.bam > $prefix1.flagstat\n";

print OUT "java -Xmx7g  -XX:ParallelGCThreads=1 -jar \$PICARDHOME/picard.jar CollectRnaSeqMetrics INPUT=$prefix1.sort.bam REFERENCE_SEQUENCE=$masterspace/MASTER.fa OUTPUT=$cwd/$prefix1.sort.PMM RIBOSOMAL_INTERVALS=Picard.ribo.intervals STRAND_SPECIFICITY=FIRST_READ_TRANSCRIPTION_STRAND REF_FLAT=Picard.ref.flat MINIMUM_LENGTH=50 CHART=$prefix1.RnaSeqMetrics.pdf\n";
...
# submit jobs
#print "bsub -P DDSBALAAE -J sh1.$prefix1 -e sh1.$prefix1.e -o sh1.$prefix1.o -q normal -n 1  -R \"rusage[mem=2500]\"  bash $prefix1.F.qc.sh1\n";
#print "bsub -P DDSBALAAE -J sh2.$prefix1 -e sh2.$prefix1.e -o sh2.$prefix1.o -q normal -n 1  -R \"rusage[mem=600]\"  bash $prefix1.F.qc.sh2\n";
print "bsub -P DDSBALAAE -J sh3.$prefix1 -e sh3.$prefix1.e -o sh3.$prefix1.o -q normal -n 2  -R \"span[ptile=2]\"  \"bash $prefix1.F.qc.sh1; bash $prefix1.F.qc.sh2; bash $prefix1.R.qc.sh3\"\n";
...
print OUT "gzip $prefix1.notCombined_2.fastq\n";
...
print OUT "source activate py3.6\n";
print OUT "perl /mnt/scratch/DCT/BIOINFCO/mzarowiecki/bin/perl/species.pl $prefix1.species > $prefix1.spec\n";
print OUT "perl /mnt/scratch/DCT/BIOINFCO/mzarowiecki/bin/perl/qc_premapping.pl $in1 $in2\n";
...
print OUT "mv $prefix1\*.F.qc.fq.gz $prefix1.QC\n";
print OUT "mv $prefix1\*.R.qc.fq.gz $prefix1.QC\n";
...
print OUT "mv $prefix1\*len $prefix1.QC\n";
print OUT "mv $prefix1\*parsed $prefix1.QC\n";
print OUT "mv $prefix2\*.R.FQC $prefix1.QC\n";
print OUT "mv $prefix1\*.F.FQC $prefix1.QC\n";
...
print OUT "mv $prefix1\*notCombined_2.fastq.gz $prefix1.QC/old\n";
