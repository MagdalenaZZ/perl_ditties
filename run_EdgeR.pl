...
if (@ARGV < 1) {
	print "\n\nUsage: run_EdgeR.pl merged.reads my.design <included columns> <prefix> <design> <cut-off>\n\n" ;

    print " mz3 script for taking eXpress_read_counts and turning them into a EdgeR DE analysis \n\n";
    print "Suitable for interaction terms\n\n";
    print '
    merged.reads : matrix with all samples in columns and genes in rows
    my.design : design matrix
    <included columns> : which columns from merged.reads will be included in the analysis, i.e. 1,2,4,6
    <prefix> : give your analysis a spiffy name
    <design> : the variables you want to test, ex. gene expression as function of Treatment, put  "~Treatment"
    <cut-off> : Exclude genes with counts below x per million (higher value, more genes excluded)
...
print R "targets =  read.table( \"$pre.design\", header=TRUE, row.names=1 )\n";
...
print R "y <- DGEList(counts=CountTable, group=targets\$$de)\n";
print R "colnames(y) <- targets\$Sample\n";
print R "dim(y)\n";

# Exclude genes with counts below 3 per million
print R "keep <- rowSums(cpm(y)>$cut, na.rm=TRUE) >= $cut\n";
print R "y <- y[keep,]\n";
print R "dim(y)\n";
...
print R "        pdf(\"$pre.plotMDS.pdf\")\n";

print R 'plotMDS(y, pch = 21, cex = 0.5)
        dev.off()
...
#y$samples$group <-relevel(y$samples$group, ref="o")
#y$samples$group <-relevel(y$samples$group, ref="p")

                # Setup design matrix
                design <- model.matrix(~0+group, data=y$samples)

                fit <- glmFit(y, design)
		levs1 <- paste("group", levels(y$samples$group)[1],sep="")
		levs2 <- paste("group", levels(y$samples$group)[2],sep="")
		levs <- paste(levs1,levs2,sep="-")
		newContrast <- makeContrasts(levs, levels=design)
...
print R "       write.table(cpm(y), \"$pre.cpm.dat\")\n";

print R '
                #print OUT fc, q values
';

print R "        write.table(tt, \"$pre.res.dat\")\n";
print R "        pdf(\"$pre.ma_plot.pdf\")\n";
...
print R "attr(dispersionFunction(dds),\"dispPriorVar\")\n";
        ...
        targets <- readTargets("testout/targ.dat")

        targets

        #Read in counts
        x <- read.delim("merged.reads2", , row.names=1, stringsAsFactors=FALSE)
        ...
        keep <- rowSums(cpm(y)>3, na.rm=TRUE) >= 3
...
y$samples$group <-relevel(y$samples$group, ref="p")
		...
		newContrast <- makeContrasts(groupo-groupp, levels=design)
		...
		# Print out CPMs
                write.table(cpm(y), "testout/cpm.dat")

                #print OUT fc, q values
                write.table(tt, "testout/res.dat")

		pdf("testout/ma_plot.pdf")
                plotSmear(lrt)
                abline(h=c(0, 0), col="blue")
                dev.off()
...
my %counts;
# %counts { gene } { prefix } = count; 

foreach my $file (@paths) {

    # make a prefix of that file name
    ...
    foreach my $ele (@in) {
        ...
        if (exists $count{$arr[1]}{$prefix} ) {
            $count{$arr[1]}{$prefix}=+ $arr[6];
        }
        else {
            $count{$arr[1]}{$prefix}=$arr[6];
        }
...
foreach my $gene (sort keys %counts) {
    #print "$gene";
    foreach my $lib (sort keys %{$counts}{$gene}) {
        print "\t$lib";
    }
    print "\n";
}
...
=pod
# redo pathway anaylsis

print R "CountTableP = read.table( \"$pre.tab.entrez\", header=TRUE, row.names=1 )\n";
print R "designP =  read.table( \"$pre.design\", header=TRUE, row.names=1 )\n";
print R "ddsP<-DESeqDataSetFromMatrix(countData= CountTableP,colData= designP,design=$de)\n";
print R "## releveling\n";
print R "try(ddsP\$Trans <- relevel(ddsP\$Trans, \"normal\"))\n";
print R "try(ddsP\$Cell <- relevel(ddsP\$Cell, \"LSK\"))\n";
print R "try(ddsP\$Hox <- relevel(ddsP\$Hox, \"hasHox\"))\n";
print R "try(ddsP\$BCat <- relevel(ddsP\$BCat, \"WT\"))\n";
print R "try(ddsP\$Biol <- relevel(ddsP\$Biol, \"A\"))\n";
print R "try(ddsP\$Virus <- relevel(ddsP\$Virus, \"Puro\"))\n";
print R "\n";
print R "ddsP <-DESeq(ddsP)\n";
print R "res <-results(ddsP, addMLE=TRUE )\n";
...
pathpv.out.list <- sapply(path.ids.all, function(pid) pathview(gene.data = exp.fc, pathway.id = pid,  out.suffix=Hoxa9.Hox.Hox.gage, 
species = "mmu", low = list(gene = "seagreen3", cpd = "blue"), mid = list(gene = "lightgoldenrod", cpd = "lightgoldenrod"), high = list(gene = "indianred1", cpd ="yellow"), limit=list(gene=20, cpd=20) ))


pathpv.out.list <- sapply(path.ids.all, function(pid) pathview(gene.data = exp.fc, pathway.id = pid,  out.suffix=Hoxa9.Hox.Hox.gage, species = "mmu", low = list(gene = "seagreen3", cpd = "blue"), mid = list(gene = "lightgoldenrod", cpd = "lightgoldenrod"), high = list(gene = "indianred1", cpd ="yellow"), limit=list(gene=20, cpd=20) ))
