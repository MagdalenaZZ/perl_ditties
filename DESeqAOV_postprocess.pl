...
system "cat $dir | cut -f2- | sed \'s/\"//g\' | tr \' \' \'\\t\' | sed \'s/baseMean/ID#baseMean/\' | tr '#' '\t' > $dir.scores.dat";


# make nice down
system "cat $dir.scores.dat | grep -v logCPM | awk '\$11\<0.05' | awk '\$6\~\"-\"' | awk '\{print \$0\"\\tDOWN\"\}'  > $dir.aovDOWN";
system "cat $dir.aovDOWN | cut -f1 | awk \'{print \$1}\' > $dir.aovdown.list ";
system "cat $dir.aovDOWN |   awk  '\$6\<-2' > $dir.aovDOWN2";
system "cat $dir.aovDOWN2 | cut -f1 | awk \'{print \$1}\' > $dir.aovdown2.list ";
...
# make nice up 
system "cat $dir.scores.dat | grep -v logCPM | awk '\$11\<0.05' | awk '\$6!\~\"-\"' | awk '\{print \$0\"\\tUP\"\}' > $dir.aovUP";
system "cat $dir.aovUP | cut -f1  | awk  \'{print \$1}\' > $dir.aovup.list ";
system "cat $dir.aovUP |   awk  '\$6\>2' > $dir.aovUP2";
system "cat $dir.aovUP2 | cut -f1  | awk  \'{print \$1}\' > $dir.aovup2.list ";
...
# make nice non-sign
system "cat $dir.scores.dat | awk '\$11\>0.05'  | awk '\{print \$0\"\\tNS\"\}' > $dir.aovNS";
