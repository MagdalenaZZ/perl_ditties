...
print R "pasillaCountTable<-read.table(\"$in2.tmp\", header=TRUE, row.names=1,  sep=\"\\t\")\n";
...
print R " pdf(\"$in2.genes_clustered.pdf\", useDingbats=FALSE)\n";
...
print R '
#order = order(rowMeans(counts(cdsFull)), decreasing=TRUE)
order = order(rowMeans(exprs(vsdFull)), decreasing=TRUE)
