...
if (@ARGV < 1) {
	print "\n\nUsage: DESeq2.pl merged.reads my.design <included columns> <prefix> <design> <file with genes> \n\n" ;
...
=pod
# Fix the first line of the input
foreach my $ne (@des) {
	chomp $ne;
...
# Remove the rows not used from the design
foreach my $ele (@inc) {
	chomp $ele;
    my $ne = $des[$ele];
    $ne=~s/^\t//;
