...
# make nice down
system "cat $dir.scores.dat | grep -v logCPM | awk '\$8\<0.05' | awk '\$6\~\"-\"' | awk '\{print \$0\"\\tDOWN\"\}'  > $dir.DOWN";
system "cat $dir.DOWN | cut -f1 | awk \'{print \$1}\' > $dir.down.list ";
system "cat $dir.DOWN |   awk  '\$6\<-2' > $dir.DOWN2";
system "cat $dir.DOWN2 | cut -f1 | awk \'{print \$1}\' > $dir.down2.list ";
...
# make nice up 
system "cat $dir.scores.dat | grep -v logCPM | awk '\$8\<0.05' | awk '\$6!\~\"-\"' | awk '\{print \$0\"\\tUP\"\}' > $dir.UP";
system "cat $dir.UP | cut -f1  | awk  \'{print \$1}\' > $dir.up.list ";
system "cat $dir.UP |   awk  '\$6\>2' > $dir.UP2";
system "cat $dir.UP2 | cut -f1  | awk  \'{print \$1}\' > $dir.up2.list ";
...
# make nice non-sign
system "cat $dir.scores.dat | awk '\$8\>0.05'  | awk '\{print \$0\"\\tNS\"\}' > $dir.NS";
