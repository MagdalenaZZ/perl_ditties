...
if (@ARGV < 1) {
	print "\n\nUsage: eXpress2DESeq2.pl merged.reads my.design <included columns> <prefix> <design> <file with genes> \n\n" ;
...
print R "dds <-DESeq(dds, test=\"LRT\",  reduced=~$ind[1])\n";
...
# Get some basic stuff
	    ...
	    print R "plotPCA(vsd,intgroup=c(\"Celltype\", \"Donor\"))\n";
	    print R "dev.off()\n\n";
...
#
		...
		#}
			...
			$had{$el3}=1;
			print "Test $el3\n";			
            		print R "\n\n# Starting to test interaction $el3\n";
            		print R "\n";
            		#print R "res <- results(dds, contrast=list(c(resultsNames(dds)[$key],resultsNames(dds)[$key2])))\n";
            		print R "res <- results(dds, name=resultsNames(dds)[$el3], test=\"LRT\")\n";
            		print R "mcols(res, use.names=TRUE)\n";
            		print R "summary(res)\n";
            		print R "resOrdered<-res[order(res\$stat),]\n";
            		print R "write.table(as.data.frame(resOrdered),file=\"$pre.$el3.LRT.dat\", sep=\"\\t\")\n";
            		print R "pdf(file=\"$pre.$el3.MA.pdf\", useDingbats=FALSE)\n";
            		print R "plotMA(res,main=\"$pre.$el3\",ylim=c(-2,2))\n";
            		print R "dev.off()\n\n";
			print R "top500<-head(rownames(resOrdered), 500)\n";
			print R "chose <- vsd[top500,]\n";
            		print R "pdf(file=\"$pre.$el3.PCA.pdf\", useDingbats=FALSE)\n";
			...
			print R "pdf(file=\"$pre.$el3.barplot.up.pdf\", useDingbats=FALSE)\n";
			...
			print R "lab <- paste(design\$Celltype, design\$Transformation, \".\")\n";
			print R "pdf(file=\"$pre.$el3.barplot.down.pdf\", useDingbats=FALSE)\n";
