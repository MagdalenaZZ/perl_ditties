...
=pod
#open (TR, "<$pre.tab") || die "I can't open $pre.tab\n";
#open (IDX, "<mart_export.Mouse_Ensembl2Entrez.txt") || die "I can't open mart_export.Mouse_Ensembl2Entrez.txt\n";
#open (OUT3, ">$pre.tab.entrez") || die "I can't open $pre.tab.entrez\n";
...
while (<TR>) {
    ...
    # catch header
    if ($_=~/^ID/) {
        print OUT3 "ID\tEntrez\n";
    }
    elsif (exists $trans{$tmp[0]} ) {
              my $new = $_;
              $new=~s/$tmp[0]/$trans{$tmp[0]}/;
              #my $new = shift @tmp;
              #print OUT3 "$trans{$tmp[0]}\t@tmp\n";
              #print OUT3 "$new\n";
              print OUT3 "$tmp[0]\t$trans{$tmp[0]}\n";
...
# check if there are any genes we should check
my @genes;

if (-s "$genes") {
    open (GEN, "<$genes") || die "I can't open $genes\n";
    while (<GEN>) {
        chomp;
        push(@genes, $_);
    }
