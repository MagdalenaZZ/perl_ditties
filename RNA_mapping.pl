...
Usage: qc_mapping.pl mapper trim-start1  (trim-start2 mate-dist mate-dev) library-type  <reference genome> gff file.fastq (file2.fastq)
...
# Do paired for mapped
	...
	#	print OUT "rm -f $prefix.sam\n";
		...
		print "module  add star/2.5.1b; module add star-fusion/1.0.0\n";
		print "\nSTAR --genomeDir /xgs-tools/production/databases/genomes_star_II/star_hg19_noChr_len101 --sjdbFileChrStartEnd /xgs-tools/production/databases/genomes_star_II/star_hg19_noChr_len101 --readFilesIn $trimout/$prefix.F\_val_1.fq.gz $trimout/$prefix.R\_val_2.fq.gz --runThreadN 14 --outFileNamePrefix /scratch/813458/AS-200819-LR-30695_paired. --genomeLoad NoSharedMemory --alignIntronMax 500000 --alignMatesGapMax 500000 --outSAMunmapped Within --outFilterMultimapNmax 1 --outFilterMismatchNmax 3 --outFilterMismatchNoverLmax 0.3 --sjdbOverhang 100 --chimSegmentMin 15 --chimScoreMin 1 --chimScoreJunctionNonGTAG 0 --chimJunctionOverhangMin 15 --readFilesCommand zcat --outStd SAM\n\n";

	}
	else {
	
		print "The mapper you have chosen $mapper, does not look like any of our accepted alterinatives; topha2, star \n";
		exit;
	}
...
if ($in2=~/SINGLE/) {
	...
	print OUT "module add python/2.7.11\nmodule add htseq/0.6.1\n\n";
	print OUT "htseq-count -f bam -r pos -s reverse -t exon -i gene_id $prefix.TH2p.Q30.bam $gff   > $prefix.htp\n"; 
 	print OUT "htseq-count -f bam -r pos -s reverse -t exon -i gene_id $prefix.TH2s.Q30.bam $gff   > $prefix.hts\n"; 
...
print OUT "paste $prefix.htp $prefix.hts | awk '{print \$1\"\\t\"\$2+\$4}' | grep -v '__' >  $prefix.htp.count\n";
...
#print OUT "source deactivate\n";
print OUT "module add bedtools/2.25.0\n";
...
print OUT "mv $prefix.F_unpaired_1.fq.gz $prefix/nonessential\n";
print OUT "mv $prefix.R_unpaired_2.fq.gz $prefix/nonessential\n";
...
print OUT "mv $prefix.hts $prefix/nonessential\n";
print OUT "mv $prefix.htp $prefix/nonessential\n";
print OUT "mv $prefix.htp.count $prefix\n";
...
print "Submit job with:\n\nbsub -P DDSBALAAE -J $prefix  -e $prefix.e -o $prefix.o -q normal -n 2 bash $prefix.mapping.sh \n\n";
