...
print R "Design = data.frame( row.names = colnames(CountTable ), condition = condition,libType = libType)\nDesign\n";
...
foreach my $key (keys %dist) {
	...
	foreach my $key2 (keys %dist) {
		...
		if ($el=~/^$el2$/ || $el>$el2) {
			...
			print R "res <-nbinomTest(cds,\"$key\",\"$key2\")\n";
	...
	print R 'test.mod1 = lm(Expression ~ Group, data = de3)
		summary(aov(test.mod1))
		res[res$id==gene,9] <- summary(aov(test.mod1))[[1]]$\'F value\'[1]
		res[res$id==gene,10] <- summary(aov(test.mod1))[[1]]$\'Pr(>F)\'[1]
