...
This script does not qsub very well so run it or Rosalind interactive queue: qrsh  
Example:  qrsh -l h_vmem=10G
Then: 
perl GSEA_preranked.pl 1000 ENSMUSG_ID_tab_log2foldc.4gsea /users/k1470436/brc_scratch/REFERENCES/msigdb.v5.1.symbols.gmt n &

    or if in emergency on a head-node.
...
default is "msigdb.v5.0.symbols.gmt"
...
print "\njava -Xmx2048m  -cp ~/bin/gsea2-2.2.4.jar xtools.gsea.GseaPreranked -gmx $gs -collapse false -mode Max_probe -norm meandiv -nperm $n -rnk $rank -scoring_scheme weighted -rpt_label my_analysis -chip /users/k1470436/scratch/REFERENCES/gsea_home/annotations/GENE_SYMBOL.chip -include_only_symbols true -make_sets true -plot_top_x 50 -rnd_seed timestamp -set_max 5000 -set_min 4 -zip_report false -out $rank\_OUT -gui false > $rank.err \n\n";
