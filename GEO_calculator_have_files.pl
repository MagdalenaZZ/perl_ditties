...
die 'Usage: GEO_calculator.pl GSE*_series_matrix.txt.gz GPL*_family.soft.gz  <matrix-factor>

Takes an existing series-matrix and GPL annotatation-file and calculates DE


Example:


perl ~/bin/perl/GEO_calculator.pl 
...
my $filename=shift;
my @a = split(/_/,$filename);
my $geonum=$a[0];
my $gplfile=shift;
my @b = split(/_/,$gplfile);
my $platform=$b[0];
...
#}
	...
	print R "GPL <- getGEO(filename=\"$gplfile\" )\n";
	print R "gset <- getGEO(filename=\"$filename\", getGPL=false)\n";
...
#print R "if (length(gset) > 1) idx <- grep(\"$platform\", attr(gset, \"names\")) else idx <- 1\n";
